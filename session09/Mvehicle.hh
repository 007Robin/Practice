...
class TrafficSim{
	...
	/*	TrafficSim(const TrafficSim& orig) {
			for(int i = 0; i < orig.vehicles.size(); i++) {
				vehicles.push_back(orig.vehicles[i]);
			}
		}
	*/
		void addCar() {
			//vehicles[n++] = new Car("red", 55, 1200);
			vehicles.push_back(new Car("red", 55, 1200));
		}
		void addBus() {
			//vehicles[n++] = new Bus(30, 15, 1200);
			vehicles.push_back(new Bus(30, 15, 1200));
		}
		void payToll() 
		{
			for (int i = 0; i < vehicles.size(); i++) {
